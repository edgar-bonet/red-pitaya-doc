...
.. tabs::
    ...
    .. group-tab:: Network
        ...
        * **Save capture time** - Select whether to save the capture time in the file name or not. The capture time is saved in the format: *YYYY-MM-DD HH:MM:SS*.
          This option also saves the capture time of individual samples (works for all format types).
...
The **Save capture time** option allows the user to include the capture time in the file name and save the capture time of individual samples. The capture time is saved in the format: *YYYY-MM-DD HH:MM:SS*. 
This feature is useful for correlating data with specific events or for post-processing analysis. The timestamp information is collected from the timestamp register in the FPGA, the exact resolution depends on
the core clock of the FPGA, but is typically in the order of nanoseconds.
